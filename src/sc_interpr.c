...
static void xCleanUpLoops(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	TpsLoop prev, this;
...
static int findloop(ret, label, sc_pb)TpsLoop* ret;
									  TscOperand label;
									  struct sc_ProcessBlock* sc_pb;
{
	TpsLoop walk;
	...
	if(walk == NULL) {
		...
		newloop->l_index = 0;
		newloop->l_retaddr =
				sc_pb->pc - (sizeof(TscOpcode) + sizeof(TscOperand));
		/*
...
static int init(self, sc_pb)object* self;
							struct sc_ProcessBlock* sc_pb;
{
	object* string;
	int datasize;
...
static void reinit(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	int i;
...
static void UnInit(self, sc_pb)object* self;
							   struct sc_ProcessBlock* sc_pb;
{
	int i;
...
static object* stacktop(sc_pb)struct sc_ProcessBlock* sc_pb;
{
 ...
 */
static int xPush(element, sc_pb)object* element;
								struct sc_ProcessBlock* sc_pb;
{
 ...
 *      Perform the RPC.
 */
static int xTrans(self, cmd, sc_pb)object* self;
								   TscOperand cmd;
								   struct sc_ProcessBlock* sc_pb;
{
	short ret;
...
#endif
	ret = trans(
			&sc_pb->hdr, sc_pb->buffer, (bufsize) sc_pb->bp, &sc_pb->hdr,
			sc_pb->buffer, (bufsize) sc_pb->maxbufsize);
#ifdef SCDEBUG
 ...
 *      Test the tuple size.
 */
static int xTTupleS(size, sc_pb)TscOperand size;
								struct sc_ProcessBlock* sc_pb;
{
	object* tuple;
 ...
 *      Unpack a tuple
 */
static int xUnpack(size, sc_pb)TscOperand size;
							   struct sc_ProcessBlock* sc_pb;
{
 ...
 */
static int xAilword(headerfield, sc_pb)TscOperand headerfield;
									   struct sc_ProcessBlock* sc_pb;
{

	switch(headerfield) {

		case H_EXTRA:sc_pb->hdr.h_extra = (uint16) _ailword;
			return 0;

		case H_SIZE:sc_pb->hdr.h_size = (bufsize) _ailword;
			return 0;

		case H_OFFSET:sc_pb->hdr.h_offset = (int32) _ailword;
			return 0;
...
static int xStringS(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	object* size, * string;
	int s;
 ...
 *
 */
static int xListS(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	object* size, * list;
	int s;
 ...
 */
static int xPutFS(size, sc_pb)TscOperand size;
							  struct sc_ProcessBlock* sc_pb;
{
	object* string;
	char* str;
 ...
 */
static int xTStringSeq(size, sc_pb)TscOperand size;
								   struct sc_ProcessBlock* sc_pb;
{
	object* string;
...
static int xTStringSlt(size, sc_pb)TscOperand size;
								   struct sc_ProcessBlock* sc_pb;
{
	object* string;
...
static int xTListSeq(size, sc_pb)TscOperand size;
								 struct sc_ProcessBlock* sc_pb;
{
	object* list;
...
static int xTListSlt(size, sc_pb)TscOperand size;
								 struct sc_ProcessBlock* sc_pb;
{
	object* list;
...
static int xPutVS(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	object* string;
	char* str;
	int size;
 ...
 */
static int xLoopPut(label, sc_pb)TscOperand label;
								 struct sc_ProcessBlock* sc_pb;
{
	TpsLoop this;
	object* list, * element;
...
static int xLoopGet(label, sc_pb)TscOperand label;
								 struct sc_ProcessBlock* sc_pb;
{
	TpsLoop this;
...
static int xEndLoop(label, sc_pb)TscOperand label;
								 struct sc_ProcessBlock* sc_pb;
{
	TpsLoop this;
...
static int xPutI(flags, sc_pb)TscOperand flags;
							  struct sc_ProcessBlock* sc_pb;
{
	object* integer;
	...
	if(flags & ALL_FIELDS) {
		...
		switch(flags & ALL_FIELDS) {
			...
			case H_EXTRA:sc_pb->hdr.h_extra = (uint16) i;
				break;

			case H_SIZE:sc_pb->hdr.h_size = (bufsize) i;
				break;

			case H_OFFSET:sc_pb->hdr.h_offset = (int32) i;
				break;
...
static int xPutC(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	capability cap;
	object* capobj;
...
static int xDup(n, sc_pb)TscOperand n;
						 struct sc_ProcessBlock* sc_pb;
{
...
static int xPop(n, sc_pb)TscOperand n;
						 struct sc_ProcessBlock* sc_pb;
{
	int i;
...
static int xAlign(n, sc_pb)TscOperand n;
						   struct sc_ProcessBlock* sc_pb;
{
...
static int xPack(size, sc_pb)TscOperand size;
							 struct sc_ProcessBlock* sc_pb;
{
...
static int xGetVS(sc_pb)struct sc_ProcessBlock* sc_pb;
{
...
static int xGetFS(size, sc_pb)TscOperand size;
							  struct sc_ProcessBlock* sc_pb;
{
	object* string;
...
static int xGetI(flags, sc_pb)TscOperand flags;
							  struct sc_ProcessBlock* sc_pb;
{
	object* integer;
	long i;
	...
	if(flags & ALL_FIELDS) {
		...
		switch(flags & ALL_FIELDS) {
			...
			case H_EXTRA:i = (long) sc_pb->hdr.h_extra;
				break;

			case H_SIZE:i = (long) sc_pb->hdr.h_size;
				break;

			case H_OFFSET:i = (long) sc_pb->hdr.h_offset;
				break;
							  ...
							  struct sc_ProcessBlock* sc_pb;
{
	...
	if((int) flags != 0) {
		switch((int) flags) {
			...
			case PSEUDOFIELD:cap.cap_port = sc_pb->hdr.h_port;
...
static int xEqual(sc_pb)struct sc_ProcessBlock* sc_pb;
{
	object* int1, * int2;
...
object* sc_interpreter(self, args)object* self, * args;
{
	TscOpcode opcode;
	TscOperand operand;
	int datasize, ret;
	...
	while(sc_pb.pc < datasize) {
		...
		switch(opcode) {
			...
			case BufSize:UnInit(self, &sc_pb);
				return err_scerr(TwoBufSize);

			case Trans:ret = xTrans(self, operand, &sc_pb);
				break;

			case TTupleS:ret = xTTupleS(operand, &sc_pb);
				break;

			case Unpack:ret = xUnpack(operand, &sc_pb);
				break;

			case AilWord:ret = xAilword(operand, &sc_pb);
				break;

			case StringS:ret = xStringS(&sc_pb);
				break;

			case ListS:ret = xListS(&sc_pb);
				break;

			case PutFS:ret = xPutFS(operand, &sc_pb);
				break;

			case TStringSeq:ret = xTStringSeq(operand, &sc_pb);
				break;

			case TStringSlt:ret = xTStringSlt(operand, &sc_pb);
				break;

			case PutVS:ret = xPutVS(&sc_pb);
				break;

			case TListSeq:ret = xTListSeq(operand, &sc_pb);
				break;
			...
			case LoopPut:ret = xLoopPut(operand, &sc_pb);
				break;

			case EndLoop:ret = xEndLoop(operand, &sc_pb);
				break;

			case PutI:ret = xPutI(operand, &sc_pb);
				break;

			case PutC:ret = xPutC(&sc_pb);
				break;

			case Dup:ret = xDup(operand, &sc_pb);
				break;

			case Pop:ret = xPop(operand, &sc_pb);
				break;

			case Align:ret = xAlign(operand, &sc_pb);
				break;

			case Pack:ret = xPack(operand, &sc_pb);
				break;

			case GetVS:ret = xGetVS(&sc_pb);
				break;

			case LoopGet:ret = xLoopGet(operand, &sc_pb);
				break;

			case GetFS:ret = xGetFS(operand, &sc_pb);
				break;

			case GetI:ret = xGetI(operand, &sc_pb);
				break;

			case GetC:ret = xGetC(operand, &sc_pb);
				break;
