...
static object* panel_getmember(addr, mlist, name)char* addr;
		...
		if(strcmp(l->name, name) == 0) {
			...
			switch(l->type) {
				...
				case T_LONG:v = newintobject(*(long*) addr);
					break;
				case T_FLOAT:v = newfloatobject(*(float*) addr);
					break;
				case T_STRING:
					if(*(char**) addr == NULL) {
						INCREF(None);
					...
					break;
				case T_ACTUATOR:v = newactuatorobject(*(Actuator**) addr);
					break;
				default:err_badarg();
					v = NULL;
			}
...
static int panel_setmember(addr, mlist, name, v)char* addr;
												struct memberlist* mlist;
												char* name;
												object* v;
{
		...
		if(strcmp(l->name, name) == 0) {
			...
			switch(l->type) {
				...
				case T_ACTUATOR:
					...
					break;
				default:err_setstr(SystemError, "unknown member type");
					return -1;
			}
			return 0; /* Found it */
		}
...
static object* panel_addpanel(self, args)panelobject* self;
										 object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* panel_endgroup(self, args)panelobject* self;
										 object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* panel_fixpanel(self, args)panelobject* self;
										 object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* panel_strwidth(self, args)panelobject* self;
										 object* args;
{
	object * v;
	double width;
...
static void panel_dealloc(p)panelobject* p;
{
...
static object* panel_getattr(p, name)panelobject* p;
									 char* name;
{
	object * v;
...
static int panel_setattr(p, name, v)panelobject* p;
									char* name;
									object* v;
{
	int err;
...
static void downfunc(a)Actuator* a;
{
	if(down_pend == NULL) {
		down_pend = a;
	}
...
static void activefunc(a)Actuator* a;
{
	if(active_pend == NULL) {
		active_pend = a;
	}
...
static void upfunc(a)Actuator* a;
{
	if(up_pend == NULL) {
		up_pend = a;
	}
...
static Actuator* makeactuator(name)char* name;
{
	Actuator* act;
...
static object* actuator_addact(self, args)actuatorobject* self;
										  object* args;
{
	Panel* p;
...
static object* actuator_addsubact(self, args)actuatorobject* self;
											 object* args;
{
	Actuator* a;
...
static object* actuator_delact(self, args)actuatorobject* self;
										  object* args;
{
	Panel* p;
	if(!getnoarg(args)) {
...
static object* actuator_fixact(self, args)actuatorobject* self;
										  object* args;
{
	Panel* p;
	if(!getnoarg(args)) {
...
static object* actuator_tprint(self, args)actuatorobject* self;
										  object* args;
{
	object * str;
...
static object* newactuatorobject(act)Actuator* act;
{
	actuatorobject* a;
	if(act == NULL) {
		INCREF(None);
...
static void actuator_dealloc(a)actuatorobject* a;
{
...
static object* actuator_getattr(a, name)actuatorobject* a;
										char* name;
{
...
static int actuator_setattr(a, name, v)actuatorobject* a;
									   char* name;
									   object* v;
{
	Actuator* act = a->ob_actuator;
...
static int actuator_compare(v, w)actuatorobject* v, * w;
{
...
static object* module_mkpanel(self, args)object* self;
										 object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* module_mkact(self, args)object* self;
									   object* args;
{
	object * v;
	Actuator* a;
...
static object* module_dopanel(self, args)object* self;
										 object* args;
{
	Actuator* a;
	object * v, *w;
...
static object* module_drawpanel(self, args)object* self;
										   object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* module_needredraw(self, args)object* self;
											object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* module_userredraw(self, args)object* self;
											object* args;
{
	short wid;
	if(!getnoarg(args)) {
...
static object* module_block(self, args)object* self;
									   object* args;
{
	int flag;
