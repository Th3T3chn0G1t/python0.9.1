...
object* newopenfileobject(fp, name, mode)FILE* fp;
										 char* name;
										 char* mode;
{
...
static object* file_repr(f)fileobject* f;
{
	char buf[300];
...
static object* file_close(f, args)fileobject* f;
								  object* args;
{
	if(args != NULL) {
		err_badarg();
...
static object* file_read(f, args)fileobject* f;
								 object* args;
{
	int n;
	object* v;
...
static object* file_readline(f, args)fileobject* f;
									 object* args;
{
	int n;
	object* v;
...
static object* file_write(f, args)fileobject* f;
								  object* args;
{
	int n, n2;
	if(f->f_fp == NULL) {
...
static object* file_getattr(f, name)fileobject* f;
									char* name;
{
...
typeobject Filetype = {
		OB_HEAD_INIT(&Typetype) 0, "file", sizeof(fileobject), 0,
