	...
	*flagp = WORST;         /* Tentatively. */
	...
	switch(*regparse++) {
		case '^':ret = regnode(BOL);
			break;
		case '$':ret = regnode(EOL);
			break;
		case '.':ret = regnode(ANY);
			...
			while(*regparse != '\0' && *regparse != ']') {
				if(*regparse == '-') {
					...
					if(*regparse == ']' || *regparse == '\0') {
						...
						if(class > classend + 1) FAIL("invalid [] range");
 ...
 */
int reglexec(prog, string, offset)register regexp* prog;
								  register char* string;
{
	register char* s;
	extern char* strchr();
	...
	while(scan != NULL) {
		...
		switch(OP(scan)) {
			...
			case END:return (1);      /* Success! */
				break;
			default:regerror("memory corruption");
				return (0);
				break;
		}
...
static char* regprop(op)char* op;
{
	...
	switch(OP(op)) {
		...
		case ANYOF:p = "ANYOF";
			break;
		case ANYBUT:p = "ANYBUT";
			break;
		case BRANCH:p = "BRANCH";
			break;
		case EXACTLY:p = "EXACTLY";
			break;
		case NOTHING:p = "NOTHING";
			break;
		case BACK:p = "BACK";
			break;
		case END:p = "END";
		...
		case OPEN + 9:
			sprintf(
					buf + strlen(buf), "OPEN%d", (int) (OP(op) - OPEN));
			p = NULL;
			break;
		case CLOSE + 1:
		case CLOSE + 2:
		...
		case CLOSE + 9:
			sprintf(
					buf + strlen(buf), "CLOSE%d", (int) (OP(op) - CLOSE));
			p = NULL;
			break;
		case STAR:p = "STAR";
			break;
		case PLUS:p = "PLUS";
			break;
		default:regerror("corrupted opcode");
			break;
	}
	if(p != NULL) {
		(void) strcat(buf, p);
