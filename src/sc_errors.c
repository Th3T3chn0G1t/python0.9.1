...
object* err_scerr(sc_errno)int sc_errno;
{
	switch(sc_errno) {

		case NoBufSize:
			err_setstr(
					StubcodeError, "Stubcode didn't start with BufSize");
			break;

		case TwoBufSize:
			err_setstr(
					StubcodeError, "Stubcode can't have more then one BufSize");
			break;

		case ElementIsNull:
			err_setstr(
					StubcodeError, "Trying to access an NIL object");
			break;

		case StackOverflow:err_setstr(StubcodeError, "Stack overflow");
			return NULL;

		case StackUnderflow:err_setstr(StubcodeError, "Stack underflow");
			return NULL;

		case NoEndLoop:err_setstr(StubcodeError, "LoopXXX with no EndLoop");
			return NULL;

		case BufferOverflow:err_setstr(StubcodeError, "Buffer overflow");
			return NULL;
