...
static int getmousedetail(v, ep)object* v;
								EVENT* ep;
{
	...
	return getintintarg(
			gettupleitem(v, 0), &ep->u.where.h, &ep->u.where.v) &&
...
static int getmenudetail(v, ep)object* v;
							   EVENT* ep;
{
	object* mp;
...
static int geteventarg(v, ep)object* v;
							 EVENT* ep;
{
	object* wp, * detail;
	int a[4];
	...
	switch(ep->type) {
		case WE_CHAR:
			...
			return 1;
		case WE_COMMAND:return getintarg(detail, &ep->u.command);
		case WE_DRAW:
		...
		case WE_MOUSE_DOWN:
		case WE_MOUSE_UP:
		case WE_MOUSE_MOVE:return getmousedetail(detail, ep);
		case WE_MENU:return getmenudetail(detail, ep);
		default:return 1;
	}
}
...
static object* makepoint(a, b)int a, b;
{
	object* v;
	object* w;
...
static object* makerect(a, b, c, d)int a, b, c, d;
{
	object* v;
	object* w;
	if((v = newtupleobject(2)) == NULL) {
...
static object* makemouse(hor, ver, clicks, button, mask)
...
static object* makemenu(mp, item)object* mp;
								 int item;
{
	object* v;
	object* w;
...
static void drawing_dealloc(dp)drawingobject* dp;
{
...
static object* drawing_line(dp, args)drawingobject* dp;
									 object* args;
{
	drawing_generic(dp, args, wdrawline);
}

static object* drawing_xorline(dp, args)drawingobject* dp;
										object* args;
{
	drawing_generic(dp, args, wxorline);
}

static object* drawing_circle(dp, args)drawingobject* dp;
									   object* args;
{
	int a[3];
...
static object* drawing_elarc(dp, args)drawingobject* dp;
									  object* args;
{
	int a[6];
...
static object* drawing_box(dp, args)drawingobject* dp;
									object* args;
{
	drawing_generic(dp, args, wdrawbox);
}

static object* drawing_erase(dp, args)drawingobject* dp;
									  object* args;
{
	drawing_generic(dp, args, werase);
}

static object* drawing_paint(dp, args)drawingobject* dp;
									  object* args;
{
	drawing_generic(dp, args, wpaint);
}

static object* drawing_invert(dp, args)drawingobject* dp;
									   object* args;
{
	drawing_generic(dp, args, winvert);
}

static object* drawing_cliprect(dp, args)drawingobject* dp;
										 object* args;
{
	drawing_generic(dp, args, wcliprect);
}

static object* drawing_noclip(dp, args)drawingobject* dp;
									   object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* drawing_shade(dp, args)drawingobject* dp;
									  object* args;
{
	int a[5];
...
static object* drawing_text(dp, args)drawingobject* dp;
									 object* args;
{
	int a[2];
	object* s;
...
static object* drawing_lineheight(dp, args)drawingobject* dp;
										   object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* drawing_baseline(dp, args)drawingobject* dp;
										 object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* drawing_textwidth(dp, args)drawingobject* dp;
										  object* args;
{
	object* s;
	if(!getstrarg(args, &s)) {
...
static object* drawing_textbreak(dp, args)drawingobject* dp;
										  object* args;
{
	object* s;
	int a;
...
static object* drawing_getattr(wp, name)drawingobject* wp;
										char* name;
{
...
static textobject* newtextobject(wp, left, top, right, bottom)windowobject* wp;
...
static void text_dealloc(tp)textobject* tp;
{
	if(tp->t_text != NULL) {
		tefree(tp->t_text);
	}
...
static object* text_arrow(self, args)textobject* self;
									 object* args;
{
	int code;
...
static object* text_getfocus(self, args)textobject* self;
										object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* text_getfocustext(self, args)textobject* self;
											object* args;
{
	int f1, f2;
	char* text;
...
static object* text_getrect(self, args)textobject* self;
									   object* args;
{
	if(!getnoarg(args)) {
		return NULL;
	...
	return makerect(
			tegetleft(self->t_text), tegettop(self->t_text),
			tegetright(self->t_text), tegetbottom(self->t_text));
}

static object* text_gettext(self, args)textobject* self;
									   object* args;
{
	if(!getnoarg(args)) {
		return NULL;
	}
	return newsizedstringobject(
			tegettext(self->t_text), tegetlen(self->t_text));
}

static object* text_move(self, args)textobject* self;
									object* args;
{
	int a[4];
	if(!getrectarg(args, a)) {
...
static object* text_setfocus(self, args)textobject* self;
										object* args;
{
	int a[2];
...
static object* text_replace(self, args)textobject* self;
									   object* args;
{
	object* text;
...
static struct methodlist text_methods[] = {
		"arrow", text_arrow, "draw", text_draw, "event", text_event, "getfocus",
		text_getfocus, "getfocustext", text_getfocustext, "getrect",
		text_getrect, "gettext", text_gettext, "move", text_move, "replace",
		text_replace, "setfocus", text_setfocus,
		{ NULL, NULL }           /* sentinel */
};

static object* text_getattr(tp, name)textobject* tp;
									 char* name;
{
	if(tp->t_attr != NULL) {
...
static int text_setattr(tp, name, v)textobject* tp;
									char* name;
									object* v;
{
	if(tp->t_attr == NULL) {
...
static menuobject* newmenuobject(title)object* title;
{
	int id;
	MENU* menu;
	menuobject* mp;
...
static void menu_dealloc(mp)menuobject* mp;
{
...
static object* menu_additem(self, args)menuobject* self;
									   object* args;
{
	object* text;
	int shortcut;
...
static object* menu_setitem(self, args)menuobject* self;
									   object* args;
{
	int index;
	object* text;
...
static object* menu_enable(self, args)menuobject* self;
									  object* args;
{
	int index;
	int flag;
...
static object* menu_check(self, args)menuobject* self;
									 object* args;
{
	int index;
	int flag;
...
static struct methodlist menu_methods[] = {
		"additem", menu_additem, "setitem", menu_setitem, "enable", menu_enable,
		"check", menu_check, { NULL, NULL }           /* sentinel */
};

static object* menu_getattr(mp, name)menuobject* mp;
									 char* name;
{
	if(mp->m_attr != NULL) {
...
static int menu_setattr(mp, name, v)menuobject* mp;
									char* name;
									object* v;
{
	if(mp->m_attr == NULL) {
...
static void window_dealloc(wp)windowobject* wp;
{
	if(wp->w_win != NULL) {
		...
		if(tag >= 0 && tag < MAXNWIN) {
			...
			fprintf(
					stderr, "XXX help! tag %d in window_dealloc\n", tag);
		}
		wclose(wp->w_win);
	}
	DECREF(wp->w_title);
...
static object* window_begindrawing(wp, args)windowobject* wp;
											object* args;
{
	drawingobject* dp;
	if(!getnoarg(args)) {
...
static object* window_change(wp, args)windowobject* wp;
									  object* args;
{
	int a[4];
	if(!getrectarg(args, a)) {
...
static object* window_gettitle(wp, args)windowobject* wp;
										object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* window_getwinsize(wp, args)windowobject* wp;
										  object* args;
{
	int width, height;
	if(!getnoarg(args)) {
...
static object* window_getdocsize(wp, args)windowobject* wp;
										  object* args;
{
	int width, height;
	if(!getnoarg(args)) {
...
static object* window_getorigin(wp, args)windowobject* wp;
										 object* args;
{
	int width, height;
	if(!getnoarg(args)) {
...
static object* window_scroll(wp, args)windowobject* wp;
									  object* args;
{
	int a[6];
...
static object* window_setdocsize(wp, args)windowobject* wp;
										  object* args;
{
	int a[2];
	if(!getpointarg(args, a)) {
...
static object* window_setorigin(wp, args)windowobject* wp;
										 object* args;
{
	int a[2];
	if(!getpointarg(args, a)) {
...
static object* window_settitle(wp, args)windowobject* wp;
										object* args;
{
	object* title;
...
static object* window_show(wp, args)windowobject* wp;
									object* args;
{
	int a[4];
	if(!getrectarg(args, a)) {
...
static object* window_settimer(wp, args)windowobject* wp;
										object* args;
{
	int a;
	if(!getintarg(args, &a)) {
...
static object* window_textcreate(self, args)windowobject* self;
											object* args;
{
	textobject* tp;
	int a[4];
...
static object* window_setselection(self, args)windowobject* self;
											  object* args;
{
	int sel;
	object* str;
	...
	ok = wsetselection(
			self->w_win, sel, getstringvalue(str), (int) getstringsize(str));
	return newintobject(ok);
}

static object* window_setwincursor(self, args)windowobject* self;
											  object* args;
{
	object* str;
	CURSOR* c;
...
static object* window_getattr(wp, name)windowobject* wp;
									   char* name;
{
	if(wp->w_attr != NULL) {
...
static int window_setattr(wp, name, v)windowobject* wp;
									  char* name;
									  object* v;
{
	if(wp->w_attr == NULL) {
...
static object* stdwin_open(sw, args)object* sw;
									object* args;
{
	int tag;
	object* title;
...
static object* stdwin_get_poll_event(poll, args)int poll;
												object* args;
{
	EVENT e;
	object* v, * w;
...
*/
	...
	switch(e.type) {
		case WE_CHAR: {
			...
			break;
		case WE_COMMAND:w = newintobject((long) e.u.command);
			break;
		case WE_DRAW:
			w = makerect(
					e.u.area.left, e.u.area.top, e.u.area.right,
					e.u.area.bottom);
			break;
		case WE_MOUSE_DOWN:
		case WE_MOUSE_MOVE:
		case WE_MOUSE_UP:
			w = makemouse(
					e.u.where.h, e.u.where.v, e.u.where.clicks,
					e.u.where.button, e.u.where.mask);
			break;
		case WE_MENU:
...
static object* stdwin_getevent(sw, args)object* sw;
										object* args;
{
	return stdwin_get_poll_event(0, args);
}

static object* stdwin_pollevent(sw, args)object* sw;
										 object* args;
{
	return stdwin_get_poll_event(1, args);
}

static object* stdwin_setdefwinpos(sw, args)object* sw;
											object* args;
{
	int a[2];
	if(!getpointarg(args, a)) {
...
static object* stdwin_setdefwinsize(sw, args)object* sw;
											 object* args;
{
	int a[2];
	if(!getpointarg(args, a)) {
...
static object* stdwin_getdefwinpos(wp, args)windowobject* wp;
											object* args;
{
	int h, v;
	if(!getnoarg(args)) {
...
static object* stdwin_getdefwinsize(wp, args)windowobject* wp;
											 object* args;
{
	int width, height;
	if(!getnoarg(args)) {
...
static object* stdwin_menucreate(self, args)object* self;
											object* args;
{
	object* title;
	if(!getstrarg(args, &title)) {
...
static object* stdwin_askfile(self, args)object* self;
										 object* args;
{
	object* prompt, * dflt;
	int new, ret;
...
static object* stdwin_askync(self, args)object* self;
										object* args;
{
	object* prompt;
	int new, ret;
...
static object* stdwin_askstr(self, args)object* self;
										object* args;
{
	object* prompt, * dflt;
	int ret;
...
static object* stdwin_message(self, args)object* self;
										 object* args;
{
	object* msg;
...
static object* stdwin_fleep(self, args)object* self;
									   object* args;
{
	if(!getnoarg(args)) {
		return NULL;
...
static object* stdwin_setcutbuffer(self, args)object* self;
											  object* args;
{
	int i;
	object* str;
...
static object* stdwin_getcutbuffer(self, args)object* self;
											  object* args;
{
	int i;
	char* str;
...
static object* stdwin_rotatecutbuffers(self, args)object* self;
												  object* args;
{
	int i;
	if(!getintarg(args, &i)) {
...
static object* stdwin_getselection(self, args)object* self;
											  object* args;
{
	int sel;
	char* data;
...
static object* stdwin_resetselection(self, args)object* self;
												object* args;
{
	int sel;
	if(!getintarg(args, &sel)) {
