...
void err_setval(exception, value)object* exception;
								 object* value;
{
...
void err_set(exception)object* exception;
{
	err_setval(exception, (object*) NULL);
}

void err_setstr(exception, string)object* exception;
								  char* string;
{
...
void err_get(p_exc, p_val)object** p_exc;
						  object** p_val;
{
...
object* err_errno(exc)object* exc;
{
...
void err_input(err)int err;
{
	switch(err) {
		case E_DONE:
		case E_OK:break;
		case E_SYNTAX:err_setstr(RuntimeError, "syntax error");
			break;
		case E_TOKEN:err_setstr(RuntimeError, "illegal token");
			break;
		case E_INTR:err_set(KeyboardInterrupt);
			break;
		case E_NOMEM:err_nomem();
			break;
		case E_EOF:err_set(EOFError);
			break;
		default:err_setstr(RuntimeError, "unknown input error");
			break;
	}
}
