...
static void code_dealloc(co)codeobject* co;
{
	XDECREF(co->co_code);
...
typeobject Codetype = {
		OB_HEAD_INIT(&Typetype) 0, "code", sizeof(codeobject), 0,
...
static codeobject* newcodeobject
PROTO((object
*, object *, object *, char *));

static codeobject* newcodeobject(code, consts, names, filename)object* code;
															   object* consts;
															   object* names;
															   char* filename;
{
...
/* Prototypes */
static int com_init
PROTO((struct compiling *, char *));
static void com_free
PROTO((struct compiling *));
static void com_done
PROTO((struct compiling *));
static void com_node
PROTO((struct compiling *, struct _node *));
static void com_addbyte
PROTO((struct compiling *, int));
static void com_addint
PROTO((struct compiling *, int));
static void com_addoparg
PROTO((struct compiling *, int, int));
static void com_addfwref
PROTO((struct compiling *, int, int *));
static void com_backpatch
PROTO((struct compiling *, int));
static int com_add
PROTO((struct compiling *, object *, object *));
static int com_addconst
PROTO((struct compiling *, object *));
static int com_addname
PROTO((struct compiling *, object *));
static void com_addopname
PROTO((struct compiling *, int, node *));
...
static object* parsenumber(s)char* s;
{
	extern long strtol();
	extern double atof();
	char* end = s;
...
static object* parsestr(s)char* s;
{
	object * v;
	int len;
	char* buf;
						  ...
						  node* n;
{
	...
	switch(TYPE(ch)) {
		...
		case LSQB:
			...
			break;
		case LBRACE:com_addoparg(c, BUILD_MAP, 0);
			break;
		case BACKQUOTE:com_node(c, CHILD(n, 1));
		...
		case STRING:
			...
			com_addoparg(c, LOAD_CONST, i);
			break;
		case NAME:com_addopname(c, LOAD_NAME, ch);
			break;
								   ...
								   node* n;
{
	...
	switch(TYPE(CHILD(n, 0))) {
		case LPAR:com_call_function(c, CHILD(n, 1));
			break;
		case DOT:com_select_member(c, CHILD(n, 1));
			break;
		case LSQB:com_apply_subscript(c, CHILD(n, 1));
			break;
		default:
			err_setstr(
					SystemError, "com_apply_trailer: unknown trailer type");
			c->c_errors++;
	}
}
						  ...
						  node* n;
{
	...
	for(i = 2; i < NCH(n); i += 2) {
		...
		switch(TYPE(CHILD(n, i - 1))) {
			case STAR:op = BINARY_MULTIPLY;
				break;
			case SLASH:op = BINARY_DIVIDE;
				break;
			case PERCENT:op = BINARY_MODULO;
				break;
			default:
				err_setstr(
						SystemError, "com_term: term operator not *, / or %");
				c->c_errors++;
				op = 255;
		}
		com_addbyte(c, op);
						  ...
						  node* n;
{
	...
	for(i = 2; i < NCH(n); i += 2) {
		...
		switch(TYPE(CHILD(n, i - 1))) {
			case PLUS:op = BINARY_ADD;
				break;
			case MINUS:op = BINARY_SUBTRACT;
				break;
			default:
				err_setstr(
						SystemError, "com_expr: expr operator not + or -");
				c->c_errors++;
				op = 255;
		}
		com_addbyte(c, op);
...
static enum cmp_op cmp_type(n)node* n;
{
	REQ(n, comp_op);
	/* comp_op: '<' | '>' | '=' | '>' '=' | '<' '=' | '<' '>'
			  | 'in' | 'not' 'in' | 'is' | 'is' not' */
								...
								node* n;
{
	...
	****************************************************************/
	...
	for(i = 2; i < NCH(n); i += 2) {
		...
		if(op == BAD) {
			err_setstr(
					SystemError, "com_comparison: unknown comparison op");
			c->c_errors++;
		}
...
PROTO((struct compiling *, node *, int));
											   ...
											   int assigning;
{
	...
	switch(TYPE(CHILD(n, 0))) {
		...
		case LSQB: /* '[' subscript ']' */
			...
			break;
		default:err_setstr(TypeError, "unknown trailer type");
			c->c_errors++;
	}
}
	...
	for(;;) {
		switch(TYPE(n)) {
			...
			case not_test:
				if(NCH(n) > 1) {
					err_setstr(
							TypeError, "can't assign to operator");
					c->c_errors++;
					return;
				}
				n = CHILD(n, 0);
			...
			case atom:
				switch(TYPE(CHILD(n, 0))) {
					case LPAR:n = CHILD(n, 1);
						if(TYPE(n) == RPAR) {
							/* XXX Should allow () = () ??? */
							err_setstr(
									TypeError, "can't assign to ()");
							c->c_errors++;
							return;
						}
						break;
					case LSQB:n = CHILD(n, 1);
						if(TYPE(n) == RSQB) {
							err_setstr(
									TypeError, "can't assign to []");
							c->c_errors++;
						...
						return;
					case NAME:com_assign_name(c, CHILD(n, 0), assigning);
						return;
					default:
						err_setstr(
								TypeError, "can't assign to constant");
						c->c_errors++;
						return;
				}
				break;
							  ...
							  node* n;
{
	...
	if(except_anchor) {
		...
		for(i = 3; i < NCH(n) && TYPE(ch = CHILD(n, i)) == except_clause;
				i += 3) {
			/* except_clause: 'except' [expr [',' expr]] */
			if(except_anchor == 0) {
				err_setstr(
						TypeError, "default 'except:' must be last");
				c->c_errors++;
				break;
			}
			except_anchor = 0;
						  ...
						  node* n;
{
	switch(TYPE(n)) {
		...
		case stmt:
		case flow_stmt:com_node(c, CHILD(n, 0));
			break;

		case simple_stmt:
		case compound_stmt:com_addoparg(c, SET_LINENO, n->n_lineno);
			com_node(c, CHILD(n, 0));
			break;
...
static void com_fplist
PROTO((struct compiling *, node *));
...
codeobject* compile(n, filename)node* n;
								char* filename;
{
	struct compiling sc;
	codeobject* co;
