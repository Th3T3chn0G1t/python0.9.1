    ...
    while 1:
        ...
        if Gl.alarm_set:
            if localtime % (24 * HOUR) = Gl.alarm_time:
                ...
                if Gl.alarm_cmd <> '':
                    d = posix.system(
                        Gl.alarm_cmd + ' ' + `Gl.alarm_time / 3600` + ' ' + `(Gl.alarm_time / 60) % 60` + ' &')
                Gl.change = 1
                clearall()
        if Gl.alarm_on:
...
def usage(exc, msg):
    ...
    #
    print
    'usage:', progname, '[options] [hh [mm [ss]]]'
    #
    print
    '-A r,g,b  : alarm background red,green,blue [255,0,0]'
    print
    '-a cmd    : shell command executed when alarm goes off'
    print
    '-B r,g,b  : background red,green,blue [0,0,0]'
    print
    '            (-B SCREENBG uses the default screen background)'
    print
    '-b        : suppress window border and title'
    print
    '-c cmapid : select explicit colormap'
    print
    '-d        : more debug output (implies -F, -w)'
    print
    '-F        : run in foreground'
    print
    '-f        : use full screen'
    print
    '-G intrvl : interval between chimes in minutes [60]'
    print
    '-g cmd    : shell command executed when chimes go off'
    print
    '-s        : single buffer mode'
    print
    '-w        : print various warnings'
    print
    '-n nparts : number of parts [' + `NPARTS` + ']'
    print
    '-T title  : alternate window title [\'' + TITLE + '\']'
    print
    '-t tzdiff : time zone difference [' + `TZDIFF` + ']'
    print
    ...
    print
    'on machines with < 12 bitplanes, -c and -s are forced on'
    #
    sys.exit(2)
...
def makewindow():
    ...
    if Gl.warnings:
        print
        nplanes, 'color planes,', nmaps, 'color maps'
    #
    if nplanes < 12 or Gl.colormap:
        if not Gl.colormap:
            ...
            if Gl.warnings:
                print
                'not enough color planes available',
                print
                'for RGB mode; forcing colormap mode'
                print
                'using color map number', Gl.colormap
        ...
        if needed > available(nplanes):
            ...
            print
            'still not enough colors available;',
            print
            'parts will be left white'
            print
            '(needed', needed, 'but have only',
            print
            available(nplanes), 'colors available)'
    #
    if Gl.doublebuffer:
        doublebuffer()
        gconfig()
...
def fixcolormap():
    ...
    if Gl.warnings:
        print
        'multimap mode has', nplanes, 'color planes'
    imap = Gl.colormap
...
def definecolor(rgb):
    ...
    if index < Gl.stopindex:
        ...
        if not Gl.overflow_seen:
            ...
            print
            'mclock: out of colormap entries'
            Gl.overflow_seen = 1
        return Gl.stopindex
    r, g, b = rgb
    if Gl.debug > 1: print
