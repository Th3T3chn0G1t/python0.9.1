...
def run(words):
    ...
    if hasglobchar(cmd):
        if args:
            print
            cmd, ': cannot glob pattern with arguments'
            return
        try:
    ...
    if isfile(cmd):
        if args:
            print
            cmd, ': file command expects no arguments'
            return
        do_page([cmd])
    elif isdir(cmd):
        if args:
            print
            cmd, ': directory command expects no arguments'
            return
        do_cd([cmd])
    else:
        print
        cmd, ': no such command, file or directory'
...
def do_grep(args):
    ...
    try:
        ...
        print
        'regexp.compile error for', args[0], ':', msg
        return
    for file in args[1:]:
        grepfile(prog, file)
...
def do_mv(args):
    ...
    if exists(dst):
        print
        src, ': destination already exists'
        return
    try:
        mac.rename(src, dst)
    except mac.error, msg:
